...
export function ensureSchema(): Promise<void> {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      // Phase 2: country faction alignment (wardog | warcat | null)
      await sql`ALTER TABLE nations ADD COLUMN IF NOT EXISTS faction TEXT`;
      await sql`
        DO $$ BEGIN
          ALTER TABLE nations
            ADD CONSTRAINT nations_faction_check
            CHECK (faction IS NULL OR faction IN ('wardog', 'warcat'));
        EXCEPTION WHEN duplicate_object THEN NULL;
        END $$;
      `;
      await sql`CREATE INDEX IF NOT EXISTS nations_faction_idx ON nations (faction) WHERE faction IS NOT NULL`;
      ...
      `;
      // Fix hubs if they already existed without faction
      await sql`UPDATE nations SET faction = 'wardog' WHERE tag = 'DOG' AND (faction IS NULL OR faction <> 'wardog')`;
      await sql`UPDATE nations SET faction = 'warcat' WHERE tag = 'CAT' AND (faction IS NULL OR faction <> 'warcat')`;
      // Backfill already-claimed countries (deterministic split)
      await sql`
        UPDATE nations
        SET faction = CASE WHEN (id % 2) = 0 THEN 'wardog' ELSE 'warcat' END
        WHERE is_default = FALSE
          AND faction IS NULL
          AND (
            leader_id IS NOT NULL
            OR member_count > 0
            OR EXISTS (SELECT 1 FROM nation_members nm WHERE nm.nation_id = nations.id)
          )
      `;
